...
def dump_example(dataset_name):
    ...
    file = h5py.File(os.path.join(path, "traindata",
                                  "{:}.h5".format(dataset_name)), "r")
    ...
    for (seq_idx, seq_name) in enumerate(file):
        ...
        dump_dir = os.path.join(path, '/Volumes/Heinrich_Backup/demon',
                                dataset_name + "_" + "{:05d}".format(seq_idx))
        ...
        for (f_idx, f_name) in enumerate(sequence):
            ...
            for dt_type in frame:
                ...
                if dt_type == "camera":
                    if f_idx == 0:
                        intrinsics = np.array(
                            [[img[0], 0, img[3]], [0, img[1], img[4]], [0, 0, 1]])
                    pose = np.array([[img[5], img[8], img[11], img[14]*scale], [
                                    img[6], img[9], img[12], img[15]*scale], [img[7], img[10], img[13], img[16]*scale]])
                    poses.append(pose.tolist())
                elif dt_type == "depth":
                    dimension = dataset.attrs["extents"]
                    depth = np.array(np.frombuffer(decompress(img.tobytes(
                    ), dimension[0] * dimension[1] * 2), dtype=np.float16)).astype(np.float32)
...
#    ])
    SUB_DATASET_NAMES = ([
        "scenes11_train", "rgbd_10_to_20_3d_train", "rgbd_10_to_20_handheld_train", "rgbd_10_to_20_simple_train", "rgbd_20_to_inf_3d_train", "rgbd_20_to_inf_handheld_train", "rgbd_20_to_inf_simple_train",
    ])

    # "sun3d_train_0.01m_to_0.1m", "sun3d_train_0.1m_to_0.2m", "sun3d_train_0.2m_to_0.4m", "sun3d_train_0.4m_to_0.8m", "sun3d_train_0.8m_to_1.6m",

    dump_root = os.path.join(path, '/Volumes/Heinrich_Backup/demon')
    ...
    if num_threads == 1:
        ...
        Parallel(n_jobs=num_threads)(delayed(dump_example)(scene)
                                     for scene in SUB_DATASET_NAMES)
